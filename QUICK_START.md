...
Server will start on `https://your-domain.example/remote`

### 4. Test the Application

**Create a Session:**
```bash
curl -X POST https://your-domain.example/remote/api/sessions/create \
...
```json
{
  ...
  "link": "https://your-domain.example/remote/support/ABC-123-XYZ",
  "expiresAt": "2026-02-05T18:00:00.000Z"
}
```

**Open Support Page:**
- Visit: `https://your-domain.example/remote/support/ABC-123-XYZ`
- You'll see the simple customer UI
- Session ID is auto-generated or from URL

**Generate Package:**
```bash
curl -X POST https://your-domain.example/remote/api/packages/generate \
  -H "Content-Type: application/json" \
  -d '{"technicianId": "tech123"}'
```
...
```json
{
  ...
  "directLink": "https://your-domain.example/remote/support/ABC-123-XYZ"
}
```
...
# Get session
curl https://your-domain.example/remote/api/sessions/ABC-123-XYZ
```
...
### Test File Upload
```bash
curl -X POST https://your-domain.example/remote/api/files/upload \
  -F "file=@test.txt" \
