...
const DEFAULT_SERVER_URL = process.env.SERVER_URL || process.env.SUPPORT_URL || '';
...
class SessionService {
    ...
    static async createSession({ technicianId, expiresIn = 3600, deviceId = null }) {
        try {
            ...
            try {
                const builder = new PackageBuilder(DEFAULT_SERVER_URL);
        ...
        try {
            ...
            if (existing) {
                return {
                    ...
                    link: `${DEFAULT_SERVER_URL}/support/${existing.session_id}`,
            ...
            if (device?.pending_session_id) {
                ...
                if (pendingSession && new Date() < new Date(pendingSession.expires_at)) {
                    ...
                    return {
                        ...
                        link: `${DEFAULT_SERVER_URL}/support/${pendingSession.session_id}`,
            ...
            return {
                sessionId,
                link: `${DEFAULT_SERVER_URL}/support/${sessionId}`,
        ...
        if (existingSessionId) {
            ...
            if (session && now < new Date(session.expires_at)) {
                return {
                    ...
                    link: `${DEFAULT_SERVER_URL}/support/${session.session_id}`,
                    expiresAt: session.expires_at,
                    existing: true
                };
            }
